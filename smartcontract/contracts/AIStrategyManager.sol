...
contract AIStrategyManager is Ownable, ReentrancyGuard {
    ...
    // State variables
    INeuroWealthVault public immutable neuroWealthVault;
    ...
    modifier onlyVault() {
        require(msg.sender == address(neuroWealthVault), "Only vault can call");
        _;
    }

    constructor(address _neuroWealthVault) Ownable(msg.sender) {
        neuroWealthVault = INeuroWealthVault(_neuroWealthVault);
    }
    ...
    function executeStrategy(
        ...
        address[] memory selectedProtocols = _getOptimalProtocols(
            amount,
            token
        );
        uint256[] memory allocations = _calculateAllocations(
            selectedProtocols,
            amount
        );
    ...
    function rebalancePortfolio(address user) external onlyVault nonReentrant {
        ...
        address[] memory newProtocols = _getOptimalProtocols(
            currentValue,
            strategy.depositToken
        );
        uint256[] memory newAllocations = _calculateAllocations(
            newProtocols,
            currentValue
        );
        ...
        neuroWealthVault.updatePositionValue(user, currentValue);
    ...
    function _calculateAllocations(
        address[] memory selectedProtocols,
        uint256 totalAmount
        ...
        for (uint256 i = 0; i < selectedProtocols.length; i++) {
            if (selectedProtocols[i] != address(0)) {
                totalInverseRisk +=
                    100 -
                    protocols[selectedProtocols[i]].riskScore;
            }
        }

        for (uint256 i = 0; i < selectedProtocols.length; i++) {
            if (selectedProtocols[i] != address(0)) {
                uint256 inverseRisk = 100 -
                    protocols[selectedProtocols[i]].riskScore;
    ...
    function updateProtocolData(
        ...
        require(
            msg.sender == dataOracle || msg.sender == owner(),
            "Unauthorized"
        );
        require(
